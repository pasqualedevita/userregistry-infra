...
# Usage:
#  ./flyway.sh info|validate|migrate ENV-SelfCare usrreg
#
#  ./flyway.sh migrate DEV-UserRegistry usrreg
#  ./flyway.sh migrate UAT-UserRegistry usrreg
#  ./flyway.sh migrate PROD-UserRegistry usrreg
...
user_registry_user_password=$(az keyvault secret show --name postgres-user-registry-user-password --vault-name "${keyvault_name}" -o tsv --query value)
